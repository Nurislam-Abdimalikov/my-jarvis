...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    async function loadAutostart() {
      if (typeof window !== 'undefined' && window.jarvis && window.jarvis.getAutostartStatus) {
        const status = await window.jarvis.getAutostartStatus()
        setAutostart(status)
      }
    }
    loadAutostart()
  ...
  const handleAutostartToggle = async () => {
    ...
    setAutostart(newVal)
    if (typeof window !== 'undefined' && window.jarvis && window.jarvis.setAutostartStatus) {
      await window.jarvis.setAutostartStatus(newVal)
    }
  }
  ...
  return (
    <aside className="drag-region w-[var(--width-sidebar)] shrink-0 flex flex-col bg-surface border-r border-border pb-4 px-3 gap-0.5">
      ...
      <div className="no-drag mt-auto flex flex-col gap-2">
        ...
        {/* Autostart Toggle */}
        <div className="px-2.5 py-2 rounded-[var(--radius-sm)] border border-border bg-surface flex items-center justify-between">
          <span className="text-[11px] font-semibold text-muted uppercase tracking-wider">Автозапуск</span>
          <button
            ...
            className={`w-7 h-4 rounded-full p-0.5 transition-colors duration-200 focus:outline-none cursor-pointer flex items-center
              ${autostart ? 'bg-accent justify-end' : 'bg-muted justify-start'}`}
          >
            <div className="w-3 h-3 rounded-full bg-white shadow-md transition-transform duration-200" />
          </button>
        </div>
